...
jest.mock('../common/D3MultiSeriesChart', () => (props) => (
  <div data-testid="mock-multi" data-showtotal={props.showTotal ? 'true' : 'false'} data-mode={props.mode} />
));
...
describe('CreateAnalysisChart factory', () => {
  test('expanded modal passes showTotal and mode to D3MultiSeriesChart', async () => {
    ...
    const modalMode = allMocksMode.find((el) => el.closest('.modalFrame')) || allMocksMode[allMocksMode.length - 1];
