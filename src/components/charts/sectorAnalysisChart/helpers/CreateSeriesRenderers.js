...
export default function createSeriesRenderers({
	...
	const renderRightEndLabelsForLines = () =>
		industries.map((ind, idx) => {
			...
			const top3Set =
				metricSuffix === 'volume' ? top3For2024.volume : top3For2024.count;
				...
				>
					<LabelList
						...
						content={({ x, y, value }) => {
							...
							const dyTable = isExpandedView
								? SMALLSET_LABEL_DY.expanded
								: SMALLSET_LABEL_DY.regular;
							const baseLift = isExpandedView
								? SMALLSET_LABEL_BASE_LIFT.expanded
								: SMALLSET_LABEL_BASE_LIFT.regular;

							let ySafe = clampY(
								y + baseLift + dyTable[seriesIdx % dyTable.length],
								chartDims,
								10
							);
